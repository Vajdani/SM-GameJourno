...
function StoneHarvestable.server_onCreate( self )
	...
	self.trigger = sm.areaTrigger.createBox(sm.vec3.one() * 5, self.harvestable.worldPosition, sm.quat.identity(), sm.areaTrigger.filter.character)
	self.trigger:bindOnEnter("sv_die")
end

function StoneHarvestable:sv_die(trigger, results)
	for k, v in pairs(results) do
		if v:isPlayer() then
			self:sv_onHit( 100, self.harvestable.worldPosition )
