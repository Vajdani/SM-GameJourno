...
function GlobUnit.server_onCreate( self )
	...
	if self.saved.stats == nil then
		self.saved.stats = { hp = 10, maxhp = 10, cardboardEaten = 0 } --100
	end
...
function GlobUnit.server_onFixedUpdate( self, dt )
	...
	if self.saved.stats.cardboardEaten >= CardboardPerGoop then
		...
		if SurvivalGame then
			...
			local count = 32
			for i = 1, count do
				lootList[i] = { uuid = obj_resource_glowpoop }
			end
			SpawnLoot( self.unit, lootList, self.unit.character.worldPosition + sm.vec3.new( 0, 0, 0.25 ), math.pi / 36 )
		end
		self.storage:save( self.saved )
	end
end
