...
local FoodCostPerStamina = 0.01 --0.02
local WaterCostPerStamina = 0.05 --0.1
local SprintStaminaCost = 0.7 / 80 / 4 --0.7 / 40 -- Per tick while sprinting
local CarryStaminaCost = 1.4 / 40 / 4 --1.4 / 40 -- Per tick while carrying

local FoodLostPerSecond = 100 / 3.5 / 24 / 60 / 4 --100 / 3.5 / 24 / 60
local WaterLostPerSecond = 100 / 2.5 / 24 / 60 / 4 --100 / 2.5 / 24 / 60

local BreathLostPerTick = ( 100 / 60 / 5 ) / 40 --( 100 / 60 ) / 40

local FatigueDamageHp = 0.25 * PerSecond --1 * PerSecond
local FatigueDamageWater = 2 * PerSecond --2 * PerSecond
local DrownDamage = 1 --5
local DrownDamageCooldown = 80 --40
...
function SurvivalPlayer.server_onCreate( self )
	...
	self.sv.saved.stats = self.sv.saved.stats or {
		hp = 1000, maxhp = 1000,
		food = 1000, maxfood = 1000,
		water = 1000, maxwater = 1000,
		breath = 1000, maxbreath = 1000
	}
